...
if ("ItemToolbox" in _weapons) then {
    ...
    if((speed player <= 1) && hasBikeItem && _canDo) then {
        if (s_player_deploybike < 0) then {
            s_player_deploybike = player addaction[("<t color=""#007ab7"">" + ("Deploy Bike") +"</t>"),"fixes\bike.sqf","",5,false,true,"", ""];
        };
    } else {
...
if ((cursorTarget isKindOf "Old_bike_TK_CIV_EP1") && ("PartEngine" in _mags)) then {
        hasBikeItem = true;
    } else { hasBikeItem = false;};
    if((speed player <= 1) && hasBikeItem && _canDo) then {
        if (s_player_upgrade < 0) then {
            s_player_upgrade = player addaction[("<t color=""#c300c3"">" + ("Upgrade Bike (motorcycle)") +"</t>"),"Fixes\Upgrade1.sqf","",5,false,true,"", ""];
        };
    } else {
        player removeAction s_player_upgrade;
        s_player_upgrade = -1;
};
